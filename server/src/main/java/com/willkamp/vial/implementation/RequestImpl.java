...
final class RequestImpl implements Request {
    ...
    static RequestImpl fromStringHeaders(ByteBufAllocator alloc, CharSequence path, Iterable<Map.Entry<String, String>> headers, ByteBuf body) {
        List<Map.Entry<CharSequence, CharSequence>> list = StreamSupport.stream(headers.spliterator(), false)
                .map(it -> new AbstractMap.SimpleEntry<CharSequence, CharSequence>(it.getKey(), it.getValue()))
    ...
    private RequestImpl(ByteBufAllocator alloc, CharSequence path, Iterable<Map.Entry<CharSequence, CharSequence>> headers) {
        this.alloc = alloc;
        this.path = path;
    ...
    public <T> Optional<T> bodyJson(Class<T> clazz) {
        return Optional.ofNullable(body).flatMap(it -> {
            try {
