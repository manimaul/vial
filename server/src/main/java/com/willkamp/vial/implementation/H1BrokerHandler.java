...
class H1BrokerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof FullHttpRequest) {
            ...
            if (handler != null) {
                Request request = RequestImpl.fromStringHeaders(ctx.alloc(), message.uri(), message.headers(), message.content());
                response = ((ResponseImpl) handler.handle(request, new ResponseImpl(ctx.alloc())))
                        .buildFullH1Response();
            } else {
                response = new ResponseImpl(ctx.alloc())
